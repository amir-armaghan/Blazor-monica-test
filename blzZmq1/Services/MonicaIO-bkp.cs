...
namespace blzZmq1.Services
{
    ...
    public class MonicaIObkp
    {
       /* public static string chartTypeName = "area";
        string ChartTypeName2 = @"
            title: {" + chartTypeName + @"
            text: 'Monica Answer from JSON'
            }";*/
        ...
        /*
                ...
                // is absolute path
                public static bool is_absolute_path(string p)
                {
                    return p.StartsWith("/") || p.Length == 2 && p[1] == ':' || p.Count > 2 && p[1] == ":" && (p[2] == "\\" || p[2] == "/");
                }

                // fix system separator
                public static object fix_system_separator(string path)
                {
                    path = path.Replace("\\", "/");
                    var new_path = path;
                    while (true)
